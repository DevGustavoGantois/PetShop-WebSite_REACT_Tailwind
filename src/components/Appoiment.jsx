import React from 'react';

//import image
import DogImg from '../assets/img/dogs/dog-appointment.png';

const Appoiment = () => {
  return (
    <section className='bg-yellow-secondary py-12 lg:pb-12 lg:mt-32 relative min-h-[548px]' id='marqueconsulta'>
        <div className='container mx-auto'>
          <div>
            <div className='hidden w-full max-w-[790px] mx-auto lg:flex justify-center'>
              <img src={DogImg} alt="" />
            </div>
            <form className='bg-yellow py-8 px-6 w-full max-w-[790px] h-[500px] mx-auto lg:-mt-5 rounded-[60px] text-center flex flex-col justify-start items-center gap-y-4'>
              <h2 className='h2 my-4'>Marque uma consulta</h2>
              <input type="text" className='input-control' placeholder='Nome Completo' />
              <input type="text" className='input-control' placeholder='Email' />
              <input type="text"  className='input-control' placeholder='Número de Telefone'/>
              <button className='btn w-full max-w-[514px]'>Enviar</button>
            </form>
          </div>
        </div>
    </section>
  )
}

export default Appoiment
